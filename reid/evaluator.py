...
class Evaluator(object):
    ...
    def extract(self, query_loader, gallery_loader, model, output_feature=None):
        query_features, _ = extract_features(model, query_loader, 1, output_feature)
        gallery_features, _ = extract_features(model, gallery_loader, 1, output_feature)

        return query_features, gallery_features

    def distance(self, query_features, gallery_features, query, gallery):
        distmat = pairwise_distance(query_features, gallery_features, query, gallery)
        return distmat

    def evaluate(self, distmat, query, gallery):
